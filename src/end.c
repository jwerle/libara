...
on_async_begin(ara_async_req_t *req) {
  ...
  if (0 == (self->bitfield.work & ARA_END)) {
  ...
  switch (self->status) {
    case ARA_STATUS_OPENED:
      ara_call(self, ARA_END, req, &on_done);
      return;
...
ARAboolean
ara_end(ara_t *self, ara_async_data_t *data, ara_cb *callback) {
  ...
  if (self) {
    switch (self->status) {
      case ARA_STATUS_OPENED:
        WORK(self, ARA_END, req, data, on_async_begin, on_async_end);
        break;
